...
const Trending = () => {
  ...
  return (
    <>
      <SEO
        title="Trending"
        description="Watch the most popular and trending videos on PlayBack right now."
        url="/trending"
        keywords="trending videos, popular videos, PlayBack trending, viral videos"
      />
      <div className="min-h-screen bg-[#0f0f0f] text-white p-6">
        <div className="flex items-center gap-3 mb-6">
          <div className="p-3 bg-red-600 rounded-full">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {videos.map((video, index) => (
              <div key={video._id} className="relative">
                {index < 7 && (
                  <div className={`absolute top-2 left-2 z-10 text-white text-xs px-2 py-1 rounded-full flex items-center gap-1 ${index === 0 ? 'bg-yellow-500' :
                    index === 1 ? 'bg-gray-400' :
