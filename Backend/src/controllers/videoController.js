...
const updateVideo = asyncHandler(async (req, res) => {
    ...
    if (!(title && description)) {
        throw new apiErrors(400, "title and description must be required");
    }
    ...
    // files may be in req.files due to upload.fields
    const thumbnailLocalPath = req.files?.thumbnail?.[0]?.path;
    const videoLocalPath = req.files?.videoFile?.[0]?.path;
    ...
    if (thumbnailLocalPath) {
        ...
        if (!thumbnail) {
            throw new apiErrors(400, "Failed to upload new thumbnail")
        }
    }
    ...
    if (videoLocalPath) {
        ...
        if (!videoFile) {
            throw new apiErrors(400, "Failed to upload new video file");
        }
    }
